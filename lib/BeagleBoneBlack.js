    ...
    GPIO_PINS.forEach(function(pin) {
      ...
      tasks.push(function(callback) {
        ...
        fs.exists('/sys/class/gpio/gpio' + pin, function(exists) {
          ...
          fs.writeFile('/sys/class/gpio/export', pin, {encoding: 'ascii'}, callback);
        });
      }.bind(this));
    }.bind(this));
...
BeagleBoneBlack.prototype.reset = function() {
  // will be called during process.on('exit') so only sync methods here.. :(

  // turn pins off for GPIO usage
  GPIO_PINS.forEach(function() {
    if(fs.existsSync('/sys/class/gpio/gpio' + pin)) {
      fs.writeFileSync('/sys/class/gpio/unexport', pin, {encoding: 'ascii'});
    }
  });
}
